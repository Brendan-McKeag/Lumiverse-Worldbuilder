 ...
 * WorldForge — frontend
 *
 * Drawer tab showing the active character's living world as managed World
 * Book entries, grouped by kind. Characters show on/off-stage; the player's
 * current location is highlighted. Operator can read, edit, and delete
 * entries and tune the agent. Invisible to the player inside the chat.
 * ------------------------------------------------------------------ */

interface Entity {
  id: string
  kind: 'location' | 'character' | 'faction' | 'lore' | 'event'
  name: string
  onstage?: boolean
  status?: string
  private?: boolean
...
const KIND_ORDER: Entity['kind'][] = ['location', 'character', 'faction', 'lore', 'event']
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  const status = q<HTMLElement>('.wf-status')
  const entitiesEl = q<HTMLElement>('.wf-entities')
  const detail = q<HTMLElement>('.wf-detail')
  const dName = q<HTMLInputElement>('.wf-d-name')
  const dKeys = q<HTMLElement>('.wf-d-keys')
  const dContent = q<HTMLTextAreaElement>('.wf-d-content')
  const activity = q<HTMLElement>('.wf-activity')
  const enEl = q<HTMLInputElement>('.wf-en')
  const roundsEl = q<HTMLInputElement>('.wf-rounds')
  const dirEl = q<HTMLTextAreaElement>('.wf-dir')
  ...
  function render() {
    if (!snap || snap.entities.length === 0) {
      entitiesEl.innerHTML =
        '<span class="wf-muted">No world yet. It grows as the story moves — the agent will create locations, characters, and lore.</span>'
      detail.style.display = 'none'
      return
    }
    const byKind = (k: Entity['kind']) => snap!.entities.filter((e) => e.kind === k)
    let html = ''
    for (const kind of KIND_ORDER) {
      ...
      for (const e of list) {
        ...
        if (e.private)
          pills.push(
            `<span class="wf-pill off" title="Only these characters know this">private · ${
              (e.audience ?? []).join(', ') || 'no one'
            }</span>`,
          )
    ...
    entitiesEl.querySelectorAll('.wf-card').forEach((el) =>
      el.addEventListener('click', () => select((el as HTMLElement).dataset.id!)),
    )
  }
  ...
  const unsub = ctx.onBackendMessage((raw: unknown) => {
    ...
    switch (p?.type) {
      case 'world': {
        ...
        status.textContent = charId
          ? snap && snap.entities.length
            ? `${snap.entities.length} entities · ${p.characterName ?? ''}`
            : 'World is empty'
          : 'No active character'
